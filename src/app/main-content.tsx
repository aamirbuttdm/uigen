...
export function MainContent({ user, project }: MainContentProps) {
  ...
  // The entire interactive tree (resizable panels, Radix tabs, Monaco, the
  // preview iframe) is client-only. SSR-ing it produces a useId prefix that
  // diverges from the hydrating client tree (the page is an async server
  // component streamed inside a Suspense boundary), causing hydration
  // mismatches. Render only after mount so the server emits no mismatching
  // markup.
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  if (!mounted) {
    return <div className="h-screen w-screen overflow-hidden bg-neutral-50" />;
  }
